...
import { renderGamesAsGroupedSlideshow } from '../game-manager/views/grouped-slideshow-view';
...
import { getShellStorageAdapter } from '../../desktop/src/utils/shell-storage-cache';
...
const MAX_SLIDESHOW_POOL_SIZE = 180;
...
function getEmulatorDetailsPopupActions() {
    if (!emulatorDetailsPopupActions) {
        emulatorDetailsPopupActions = createEmulatorDetailsPopupActions({
            ...
            fetchEmulators,
            getEmulatorConfig,
            saveEmulatorOverrides: (emulator, overrides) => getEmulatorConfigActions().saveEmulatorOverrides(emulator, overrides),
            normalizeEmulatorDownloadLinks,
            hasAnyDownloadLink,
...
function getGameCardElements() {
    if (!gameCardElements) {
        gameCardElements = createGameCardElements({
            ...
            removeGame: (game) => removeGame(Number(game?.id || 0)),
            emubro,
            alertUser: (message) => alert(message)
        });
    }
...
export function renderGames(gamesToRender, options = {}) {
    ...
    if (activeView === 'slideshow' && currentGroupBy !== 'none') {
        lastRenderedView = activeView;
        renderGamesAsGroupedSlideshow(gamesToRender, {
