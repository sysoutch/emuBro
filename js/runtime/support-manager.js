...
function extractSupportAssistantTaskObject(rawValue) {
    ...
    if (directTask === SUPPORT_ASSISTANT_TASK_FETCH_SPECS) {
        const confidence = Number(rawValue.confidence ?? rawValue.score ?? rawValue.probability);
        ...
        return {
            ...
            reason: String(rawValue.reason || rawValue.why || '').trim()
        };
    }
...
function parseSupportAssistantTask(answerText) {
    ...
    if (stripped.toUpperCase() === SUPPORT_ASSISTANT_TASK_FETCH_SPECS) {
        return { type: SUPPORT_ASSISTANT_TASK_FETCH_SPECS, confidence: 1, reason: '' };
    }
...
export function showSupportView() {
    ...
    const runSupportRequest = async ({ skipUserHistoryAppend = false, taskDepth = 0 } = {}) => {
        if (currentMode === 'help') {
            return;
        }
        if (voiceListening) {
        ...
        try {
            ...
            if (assistantTask?.type === SUPPORT_ASSISTANT_TASK_FETCH_SPECS) {
                ...
                const approved = window.confirm('Assistant wants to fetch your PC specs before continuing. Allow it?');
                if (!approved) {
            ...
            statusEl.textContent = t('support.status.ready', 'Support response ready ({{provider}}).', {
