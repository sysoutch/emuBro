...
const DEFAULT_WINDOW_FRAME_RADIUS = 10;
const MAXIMIZE_ICON_MARKUP = `
...
function readConfiguredWindowCornerRadius() {
    const stored = Number.parseInt(String(window.localStorage?.getItem(WINDOW_FRAME_RADIUS_STORAGE_KEY) || ''), 10);
    if (Number.isFinite(stored)) {
        return Math.max(0, stored);
    }
    if (typeof getComputedStyle === 'function') {
        const radiusVar = Number.parseFloat(
            String(getComputedStyle(document.documentElement).getPropertyValue('--radius-window-base') || '').trim()
        );
        if (Number.isFinite(radiusVar)) {
            return Math.max(0, Math.round(radiusVar));
        }
    }
    return DEFAULT_WINDOW_FRAME_RADIUS;
}
...
export function setupWindowControls(options = {}) {
    ...
    const useNativeTauriDragRegions = !!(
        emubro
        && emubro.platform !== 'web'
    ...
    if (maxBtn) maxBtn.addEventListener('click', async () => {
        ...
        scheduleMaxIconSyncBurst([0, 70, 180, 320]);
        scheduleNativeCornerSyncBurst([0, 70, 180, 320]);
    });
    ...
    const scheduleMaxIconSync = (delayMs = 110) => {
        ...
        maxIconSyncRaf = window.requestAnimationFrame(() => {
            maxIconSyncRaf = null;
            Promise.resolve(updateMaxIcon()).finally(() => {
                syncMaxButtonVisualState();
            });
        });
        if (maxIconSyncTimer) {
    ...
    const syncMaxButtonVisualState = () => {
        if (!maxBtn) return;
        const isMax = document.documentElement.getAttribute('data-window-state') === 'maximized';
        maxBtn.innerHTML = isMax ? RESTORE_ICON_MARKUP : MAXIMIZE_ICON_MARKUP;
        maxBtn.dataset.windowState = isMax ? 'maximized' : 'normal';
    };

    const scheduleMaxIconSyncBurst = (delays = [0, 70, 180, 320]) => {
    ...
    const scheduleNativeCornerSyncBurst = (delays = [0, 70, 180, 320]) => {
        delays.forEach((delayMs) => {
            window.setTimeout(() => {
                syncNativeWindowCornerRadius();
            }, Math.max(0, Number(delayMs) || 0));
        });
    };

    if (maxBtn && typeof MutationObserver !== 'undefined') {
        const windowStateObserver = new MutationObserver(() => {
            syncMaxButtonVisualState();
        });
        windowStateObserver.observe(document.documentElement, {
            attributes: true,
    ...
    if (typeof emubro.onWindowMaximizedChanged === 'function') {
        emubro.onWindowMaximizedChanged((isMax) => {
            Promise.resolve(updateMaxIcon(!!isMax)).finally(() => {
                syncMaxButtonVisualState();
            });
            scheduleMaxIconSyncBurst([0, 80, 180]);
            scheduleNativeCornerSyncBurst([0, 80, 180]);
        });
    }

    window.addEventListener('focus', () => {
        scheduleMaxIconSyncBurst([0, 40, 140]);
        scheduleNativeCornerSyncBurst([0, 40, 140]);
    });
    document.addEventListener('visibilitychange', () => {
        if (document.visibilityState === 'visible') {
            scheduleMaxIconSyncBurst([0, 30, 120]);
            scheduleNativeCornerSyncBurst([0, 30, 120]);
        }
    });
    window.addEventListener('resize', () => {
        scheduleMaxIconSync(60);
        scheduleNativeCornerSyncBurst([0, 60, 180]);
    });
    window.addEventListener('pageshow', () => {
        scheduleMaxIconSyncBurst([0, 40, 160]);
        scheduleNativeCornerSyncBurst([0, 40, 160]);
    });
    ...
    if (header && !useNativeTauriDragRegions) {
        ...
        const requestWindowDrag = (event) => {
            if (event.button !== 0) return;
            if (event.detail > 1) return;
            if (shouldCloseOpenCompactMenus(event.target)) {
                try {
                    window.dispatchEvent(new CustomEvent('emubro:close-compact-menus'));
                } catch (_error) {}
                return;
            }
...
export function setupHeaderThemeControlsToggle(options = {}) {
    ...
    const themeCompactQuery = createHeaderWidthQuery(1640);
    const languageCompactQuery = createHeaderWidthQuery(1640);
    ...
    const applyHeaderDensityMode = () => {
        ...
        if (density === 'compact' && headerWidth <= 1400) {
    ...
    const createCompactChipBlock = (title, options = []) => {
        ...
        options.forEach((entry) => {
            ...
            const leadingNode = typeof entry.createLeadingNode === 'function'
                ? entry.createLeadingNode()
                : null;
            if (leadingNode instanceof Node) {
                optionBtn.classList.add('has-leading-visual');
                optionBtn.appendChild(leadingNode);
            }
            const labelText = document.createElement('span');
            labelText.className = 'filter-pair-compact-option__label';
            labelText.textContent = String(entry.label);
            optionBtn.appendChild(labelText);
        ...
        buildBlocks: () => {
            ...
            const languageEntries = Array.from(languageOptions?.querySelectorAll?.('li[data-value]') || [])
                .map((li) => {
                    ...
                    const flagEl = li.querySelector('.fi');
                    const label = String(
                    ...
                    return {
                        ...
                        label,
                        createLeadingNode: () => {
                            if (!(flagEl instanceof HTMLElement)) return null;
                            return flagEl.cloneNode(true);
                        }
                    };
                })
                .filter(Boolean);
            ...
            if (languageEntries.length > 0) {
                blocks.push(createCompactChipBlock('Language', languageEntries.map((entry) => ({
                    ...
                    createLeadingNode: entry.createLeadingNode,
                    onSelect: () => {
    ...
    window.addEventListener('emubro:close-compact-menus', closeCompactMenus);
