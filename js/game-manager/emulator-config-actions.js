...
export function createEmulatorConfigActions(deps = {}) {
    ...
    function saveEmulatorOverrides(emulator, overrides = {}) {
        const key = getEmulatorKey(emulator);
        if (!key) return normalizeEditorConfig(overrides);
        const next = mergeConfigWithDefaults(emulator, {
            ...getStoredEmulatorConfig(emulator),
            ...normalizeStoredEditorConfig(overrides)
        });
    ...
    async function openEmulatorConfigEditor(emulator) {
        ...
        saveEmulatorOverrides(emulator, normalized);
        return true;
    }
