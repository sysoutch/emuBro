...
public class BroGameDAO implements GameDAO {
	...
	public void updateGame(Game game) throws SQLException {
		...
		LocalDateTime da = game.getDateAdded();
		LocalDateTime lp = game.getLastPlayed();
	...
	public void addGame(Game game, String filePath) throws SQLException, BroGameAlreadyExistsException, BroGameDeletedException {
		...
		LocalDateTime da = game.getDateAdded();
		LocalDateTime lp = game.getLastPlayed();
		Timestamp dateAdded = Timestamp.valueOf(da);
		Timestamp lastPlayed = (lp == null) ? null : Timestamp.valueOf(lp);
	...
	public BroGame getGameAt(int gameId) throws SQLException {
		...
		if (rset.next()) {
			...
			Timestamp dateAdded = rset.getTimestamp("game_added");
			Timestamp lastPlayed = rset.getTimestamp("game_lastPlayed");
			LocalDateTime dateTimeAddded = dateAdded.toLocalDateTime();
			LocalDateTime dateTimeLastPlayed = (lastPlayed == null) ? null : lastPlayed.toLocalDateTime();
			...
			game = new BroGame(id, name, defaultFileId, checksumId, iconPath, coverPath, rate, dateTimeAddded, dateTimeLastPlayed, playCount, defaultEmulatorId,
	...
	public List<Tag> getTags(int gameId) throws SQLException {
		...
		Statement stmt = conn.createStatement();
		stmt = conn.createStatement();
		String sql = "select * from game_tag where game_id = " + gameId
				+ " order by game_id, tag_id";
		ResultSet rset = stmt.executeQuery(sql);
		while (rset.next()) {
	...
	public void updateLastPlayed(Game game) throws SQLException {
		LocalDateTime date = game.getLastPlayed();
		Timestamp lastPlayed = (date == null) ? null : Timestamp.valueOf(date);
