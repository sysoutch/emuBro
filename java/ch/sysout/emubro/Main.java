...
public class Main {
		...
		try {
			...
			if (explorerDAO != null) {
				try {
					...
					List<BroPlatform> defaultPlatforms = initDefaultPlatforms();
					List<BroTag> defaultTags = initDefaultTags();
			...
			if (expectedVersion > currentVersion) {
				JOptionPane.showConfirmDialog(dlgSplashScreen, "Cannot open database because you are using a newer version of "+Messages.get(MessageConstants.APPLICATION_TITLE)+".\n\n"
						...
						Messages.get(MessageConstants.INITIALIZING_FAILURE), JOptionPane.YES_NO_CANCEL_OPTION, JOptionPane.WARNING_MESSAGE);
			} else {
	...
	public static List<BroPlatform> initDefaultPlatforms() throws FileNotFoundException, SQLException {
		InputStream stream = Main.class.getResourceAsStream("/platforms.json");
	...
	public static List<BroTag> initDefaultTags() throws FileNotFoundException, SQLException {
		InputStream stream = Main.class.getResourceAsStream("/tags.json");
		BufferedReader br = new BufferedReader(new InputStreamReader(stream));
