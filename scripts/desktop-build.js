...
const DESKTOP_CONF_PATH = path.join(DESKTOP_DIR, "src-tauri", "tauri.conf.json");

function readTauriVersion() {
  try {
    const raw = fs.readFileSync(DESKTOP_CONF_PATH, "utf8");
...
function runTauriBuild() {
  ...
  if (forceNsisOnly) {
    ...
    console.log(
      `[desktop-build] Windows prerelease version ${version} detected, using --bundles nsis to avoid MSI version restriction.`
    );
  }
