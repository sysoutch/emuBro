...
export function normalizeInputBindingProfile(input = {}) {
  const source = input && typeof input === "object" && !Array.isArray(input) ? input : {};
  const hasChannels = Object.prototype.hasOwnProperty.call(source, "keyboard") || Object.prototype.hasOwnProperty.call(source, "gamepad");
...
export function loadPlatformGamepadBindingsMap(storageRef = null) {
  const storage = getShellStorageAdapter(storageRef);
  try {
    const raw = storage.getItem(PLATFORM_GAMEPAD_BINDINGS_STORAGE_KEY);
    if (!raw) return {};
...
export function savePlatformGamepadBindingsMap(nextMap = {}, storageRef = null) {
  const storage = getShellStorageAdapter(storageRef);
  try {
    ...
    Object.entries(source).forEach(([platformShortName, rawBindings]) => {
      ...
      if (!key) return;
      const normalized = normalizeInputBindingProfile(rawBindings);
      if (hasAnyBindings(normalized)) {
        out[key] = normalized;
      }
    });
    storage.setItem(PLATFORM_GAMEPAD_BINDINGS_STORAGE_KEY, JSON.stringify(out));
  } catch (_error) {}
}

export function getPlatformGamepadBindings(platformShortName, storageRef = null) {
  const key = normalizePlatformKey(platformShortName);
...
export function setPlatformGamepadBindings(platformShortName, bindings = {}, storageRef = null) {
  const key = normalizePlatformKey(platformShortName);
  if (!key) return;
  const map = loadPlatformGamepadBindingsMap(storageRef);
