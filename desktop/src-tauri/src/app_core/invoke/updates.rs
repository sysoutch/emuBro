...
fn app_platform_asset_suffixes() -> Vec<&'static str> {
    if cfg!(target_os = "windows") {
        ...
        vec![".appimage", ".exe", ".msi", ".dmg", ".deb", ".rpm"]
    }
}

fn is_source_archive_asset_name(asset_name: &str) -> bool {
    let lower = asset_name.trim().to_ascii_lowercase();
    lower.starts_with("source code (zip)")
        || lower.starts_with("source code (tar.gz)")
        || lower.starts_with("source code (tar)")
}
    ...
    for suffix in suffixes {
        ...
        for asset in &assets {
            let name = asset
                ...
                .trim()
                .to_string();
            if name.is_empty() || is_source_archive_asset_name(&name) {
                continue;
            }
    ...
    if !is_supported_download_url(&download_url) {
        let state = with_config(
            json!({
                ...
                "progressPercent": 0,
                "downloadedFilePath": "",
                "lastError": "Could not resolve a compatible installer download URL for this platform. Open the release page instead.",
                "lastMessage": ""
            }),
            &checked,
        );
