...
public class PreviewPanePanel extends JPanel implements GameListener {
	...
	class SelectionPanel extends ScrollablePanel {
		...
		protected void setGameTitle(String s, Icon icon) {
			String gameTitle = FilenameUtils.removeExtension(s).replace(".", " ").replace("_", " ");
			pnlRatingBar.showCurrentRate();
		...
		class PathPanel extends ScrollablePanel {
			...
			private JLabel lblFilename = new JLabel2(
					"<html><strong>" + Messages.get("fileName") + "</strong></html>");
			private JLabel lblFileLocation = new JLabel2(
					"<html><strong>" + Messages.get("fileLocation") + "</strong></html>");
			public PathPanel() {
				initPathLink();
				createUI();
			}
